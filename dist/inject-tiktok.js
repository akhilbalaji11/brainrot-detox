...
`;class Q{constructor(){c(this,"settings");c(this,"session");c(this,"enabled",!1);c(this,"tickTimer",null);c(this,"lastSignalAt",0);c(this,"scrollCount",0);c(this,"swipeCount",0);c(this,"maxCookedShown",!1);c(this,"builtDifferentDismissed",!1)}async init(){var a;try{const e=await l({type:"GET_SETTINGS"});if(!e.success)return;if(this.settings=e.data,!this.settings.masterEnabled||!((a=this.settings.sites[this.site])!=null&&a.enabled)){console.log(`[brainrot detox] Disabled for ${this.site}`);return}const t=await this.getCurrentTabId();let i=await l({type:"GET_SESSION",payload:{tabId:t}});i.data?this.session=i.data:(this.session={site:this.site,tabId:t,startedAt:Date.now(),cookedScore:0,cookedStatus:"Based",lastInterventionAt:0,packState:{...b},touchGrass:{...k},vibeIntent:this.settings.vibeCheck.activeIntent,itemsConsumed:0,scrollEvents:0},await l({type:"UPDATE_SESSION",payload:{tabId:t,patch:this.session}})),this.enabled=!0,console.log(`[brainrot detox] ${this.site} adapter initialized`),this.mountCookedWidget(),this.setupObservers(),this.tickTimer=window.setInterval(()=>this.tick(),N),this.session.touchGrass.active&&this.session.touchGrass.endsAt>Date.now()&&this.showTouchGrassOverlay()}catch(e){console.error(`[brainrot detox] Init error for ${this.site}:`,e)}}destroy(){this.enabled=!1,this.tickTimer&&clearInterval(this.tickTimer),this.removeAllOverlays()}async tick(){if(!this.enabled)return;const a=Date.now(),e=this.getNewItemsSinceLastTick();this.session.itemsConsumed+=e,this.session.scrollEvents+=this.scrollCount;const t=this.scrollCount>0||e>0||this.swipeCount>0;t&&(this.lastSignalAt=a);const i=this.lastSignalAt===0?0:a-this.lastSignalAt;if(this.builtDifferentDismissed&&t){this.builtDifferentDismissed=!1,this.scrollCount=0,this.swipeCount=0,this.onBuiltDifferentDenied();return}let o;if(this.session.packState.active)o=this.session.cookedScore;else if(this.site==="shorts")o=W(this.session.cookedScore,this.swipeCount,this.session.vibeIntent,i);else{const r=(a-this.session.startedAt)/6e4,d=F(r,this.settings.cooked.sessionCapMinutes,this.session.scrollEvents,this.session.itemsConsumed);o=U(this.session.cookedScore,d,t,i,this.session.vibeIntent)}this.scrollCount=0,this.swipeCount=0,this.session.cookedScore=o,this.session.cookedStatus=E(o,this.settings.cooked.thresholds),this.session.packState.active&&(this.session.packState=L(this.session.packState,e),D(this.session.packState,a)&&this.onPackComplete()),this.session.packState.active||(A(o)?this.maxCookedShown||(this.maxCookedShown=!0,this.onMaxCooked()):(o<95&&(this.maxCookedShown=!1),o>=this.session.cookedScore&&M(o,this.session.lastInterventionAt,this.settings.cooked.thresholds,a)&&(this.session.lastInterventionAt=a,this.onIntervention()))),this.updateCookedWidget(this.session.cookedScore,this.session.cookedStatus);const n=this.session.tabId;await l({type:"UPDATE_SESSION",payload:{tabId:n,patch:this.session}})}async getCurrentTabId(){return new Promise(a=>{const e=Math.floor(Math.random()*1e6);l({type:"GET_CURRENT_TAB"}).then(t=>{var i;a(((i=t.data)==null?void 0:i.id)??e)})})}onIntervention(){l({type:"LOG_EVENT",payload:{eventType:"intervention"}}),this.showInterventionOverlay()}onMaxCooked(){this.session.lastInterventionAt=Date.now(),l({type:"LOG_EVENT",payload:{eventType:"intervention"}}),this.showSkyrimOverlay("Your brain is absolutely cooked (x_x)")}onPackComplete(){this.session.packState={...b},this.maxCookedShown=!1,l({type:"END_PACK",payload:{tabId:this.session.tabId}}),this.showSkyrimOverlay("[#] Pack Complete! Time to touch grass.")}onBuiltDifferentDenied(){this.showBuiltDifferentDeniedOverlay()}async startPack(a,e){await l({type:"START_PACK",payload:{tabId:this.session.tabId,mode:a,limit:e}}),this.session.packState={active:!0,mode:a,limit:e,consumed:0,startedAt:Date.now()},this.session.cookedScore=0,this.session.cookedStatus="Based",this.maxCookedShown=!1,this.builtDifferentDismissed=!1}async startTouchGrass(a){await l({type:"START_TOUCH_GRASS",payload:{tabId:this.session.tabId,minutes:a}}),this.session.touchGrass={active:!0,endsAt:Date.now()+a*6e4,bypassCount:0},this.showTouchGrassOverlay()}async endTouchGrass(){await l({type:"END_TOUCH_GRASS",payload:{tabId:this.session.tabId}}),this.session.touchGrass={...k},this.session.cookedScore=0,this.session.cookedStatus="Based",this.maxCookedShown=!1,this.removeAllOverlays()}async bypassTouchGrass(){this.session.touchGrass.bypassCount++,l({type:"LOG_EVENT",payload:{eventType:"bypass"}}),await this.endTouchGrass()}setVibeIntent(a){this.session.vibeIntent=a,l({type:"UPDATE_SESSION",payload:{tabId:this.session.tabId,patch:{vibeIntent:a}}}),l({type:"LOG_EVENT",payload:{eventType:"vibe_check"}})}}class ee extends Q{constructor(){super(...arguments);c(this,"site","tiktok");c(this,"itemsSinceLastTick",0);c(this,"watchTimeSinceLastTick",0);c(this,"lastVideoId","");c(this,"videoObserver",null);c(this,"currentVideo",null);c(this,"onWheel",e=>{Math.abs(e.deltaY)>20&&this.swipeCount++});c(this,"onKeyDown",e=>{(e.key==="ArrowDown"||e.key==="j")&&(this.swipeCount++,this.itemsSinceLastTick++)})}getTikTokVideo(){return document.querySelector("[data-e2e='recommend-list-item-container'] video")??document.querySelector(".tiktok-web-player video")??document.querySelector("video")}freezeFeed(){const e=this.getTikTokVideo();e&&!e.paused&&e.pause()}thawFeed(){const e=this.getTikTokVideo();e&&e.paused&&e.play().catch(()=>{})}setupObservers(){window.addEventListener("wheel",this.onWheel,{passive:!0}),window.addEventListener("keydown",this.onKeyDown,{passive:!0});let e=0;window.addEventListener("touchstart",t=>{var i;e=((i=t.touches[0])==null?void 0:i.clientY)??0},{passive:!0}),window.addEventListener("touchend",t=>{var o;const i=e-(((o=t.changedTouches[0])==null?void 0:o.clientY)??0);Math.abs(i)>50&&(this.swipeCount++,this.itemsSinceLastTick++)},{passive:!0}),this.watchVideoChanges(),this.trackWatchTime(),chrome.runtime.onMessage.addListener((t,i,o)=>{var n,r,d;return t.type==="END_TOUCH_GRASS"?(this.session.touchGrass={active:!1,endsAt:0,bypassCount:0},h("skyrim"),h("touchgrass"),this.thawFeed(),o({success:!0}),!1):t.type==="TRIGGER_TOUCH_GRASS"?(this.startTouchGrass(((n=t.payload)==null?void 0:n.minutes)??5),o({success:!0}),!1):t.type==="TRIGGER_PACK"?(this.startPack(((r=t.payload)==null?void 0:r.mode)??"items",((d=t.payload)==null?void 0:d.limit)??10),o({success:!0}),!1):(t.type==="TRIGGER_VIBE_CHECK"&&(this.showVibeCheckOverlay(),o({success:!0})),!1)})}watchVideoChanges(){const e=()=>{const o=this.getTikTokVideo();if(o&&o!==this.currentVideo){this.currentVideo=o;const n=o.src||o.currentSrc||String(Date.now());n!==this.lastVideoId&&(this.lastVideoId=n,this.itemsSinceLastTick++)}};setInterval(e,500);const t=new MutationObserver(()=>e()),i=()=>{const o=document.querySelector("[data-e2e='recommend-list-item-container']")??document.querySelector(".tiktok-web-player")??document.body;t.observe(o,{childList:!0,subtree:!0})};document.readyState==="complete"?i():window.addEventListener("load",i),this.videoObserver=t}trackWatchTime(){setInterval(()=>{const t=this.getTikTokVideo();t&&!t.paused&&this.isVideoInViewport(t)&&(this.watchTimeSinceLastTick+=100)},100)}isVideoInViewport(e){const t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight||document.documentElement.clientHeight)&&t.right<=(window.innerWidth||document.documentElement.clientWidth)}getNewItemsSinceLastTick(){const e=this.itemsSinceLastTick;return this.itemsSinceLastTick=0,e}getWatchTimeSinceLastTick(){const e=this.watchTimeSinceLastTick;return this.watchTimeSinceLastTick=0,e}mountCookedWidget(){this.updateCookedWidget(this.session.cookedScore,this.session.cookedStatus)}updateCookedWidget(e,t){const i=I(t),o=t==="Based"?"brd-score-based":t==="Medium Cooked"?"brd-score-medium":"brd-score-cooked";let n="";if(this.session.packState.active){const p=Y(this.session.packState);n=`
        ...
        `).querySelector(".brd-widget");d&&(d.style.cursor="pointer",d.onclick=()=>this.showVibeCheckOverlay())}showInterventionOverlay(){var i,o,n;const e=I(this.session.cookedStatus),t=g("intervention",`
        ...
        `);(i=t.querySelector("[data-action='dismiss']"))==null||i.addEventListener("click",()=>h("intervention")),(o=t.querySelector("[data-action='pack']"))==null||o.addEventListener("click",()=>{h("intervention"),this.startPack("items",10)}),(n=t.querySelector("[data-action='grass']"))==null||n.addEventListener("click",()=>{h("intervention"),this.startTouchGrass(this.settings.touchGrass.defaultMinutes)})}showBuiltDifferentDeniedOverlay(){var t,i,o;this.freezeFeed();const e=g("denied",`
            <div class="brd-fullscreen">
                <div class="brd-card">
        ...
        `),r=n.querySelector(".brd-zen-img"),d=n.querySelector(".brd-zen-webcam"),p=n.querySelector(".brd-zen-caption");let v=null,S=0;const G=["breathe.","you are here.","it's okay.","look at this.","touch grass.","be present.","slow down.","this is real life."],C=async m=>{const y=o[m%o.length];if(y==="WEBCAM"){if(r.style.display="none",d.style.display="block",p.textContent="hi. this is you.",!v)try{v=await navigator.mediaDevices.getUserMedia({video:!0,audio:!1}),d.srcObject=v}catch{C(m+1)}}else d.style.display="none",r.style.display="block",r.src=y,p.textContent=G[Math.floor(Math.random()*G.length)]};C(S);const z=setInterval(()=>{S++,C(S)},4e3),P=n.querySelector("#brd-tg-timer"),R=setInterval(()=>{const m=Math.max(0,e-Date.now()),y=Math.floor(m/6e4),te=Math.floor(m%6e4/1e3);P&&(P.textContent=`${String(y).padStart(2,"0")}:${String(te).padStart(2,"0")}`),m<=0&&(clearInterval(R),clearInterval(z),v&&v.getTracks().forEach(se=>se.stop()),this.endTouchGrass(),this.thawFeed(),h("touchgrass"))},1e3);(V=n.querySelector("[data-action='bypass']"))==null||V.addEventListener("click",()=>{clearInterval(R),clearInterval(z),v&&v.getTracks().forEach(m=>m.stop()),this.bypassTouchGrass(),this.thawFeed(),h("touchgrass")})}showVibeCheckOverlay(){var i;const e=H.map(o=>`
        ...
        `);t.querySelectorAll("[data-vibe]").forEach(o=>{o.addEventListener("click",()=>{const n=o.dataset.vibe;this.setVibeIntent(n),h("vibecheck")})}),(i=t.querySelector("[data-action='skip']"))==null||i.addEventListener("click",()=>{h("vibecheck")})}async tick(){if(!this.enabled)return;const e=Date.now(),t=this.getNewItemsSinceLastTick();this.session.itemsConsumed+=t,this.session.scrollEvents+=this.scrollCount;const i=this.scrollCount>0||t>0||this.swipeCount>0;i&&(this.lastSignalAt=e);const o=this.lastSignalAt===0?0:e-this.lastSignalAt;if(this.builtDifferentDismissed&&i){this.builtDifferentDismissed=!1,this.scrollCount=0,this.swipeCount=0,this.onBuiltDifferentDenied();return}const n=this.getWatchTimeSinceLastTick();let r;this.session.packState.active?r=this.session.cookedScore:r=K(this.session.cookedScore,n,this.swipeCount,this.session.vibeIntent,o),this.scrollCount=0,this.swipeCount=0,this.session.cookedScore=r,this.session.cookedStatus=E(r,this.settings.cooked.thresholds),this.session.packState.active&&(this.session.packState=L(this.session.packState,t),D(this.session.packState,e)&&this.onPackComplete()),this.session.packState.active||(A(r)?this.maxCookedShown||(this.maxCookedShown=!0,this.onMaxCooked()):(r<95&&(this.maxCookedShown=!1),r>=this.session.cookedScore&&M(r,this.session.lastInterventionAt,this.settings.cooked.thresholds,e)&&(this.session.lastInterventionAt=e,this.onIntervention()))),this.updateCookedWidget(this.session.cookedScore,this.session.cookedStatus),await l({type:"UPDATE_SESSION",payload:{tabId:this.session.tabId,patch:this.session}})}removeAllOverlays(){Z(),this.thawFeed()}}console.log("[brainrot detox] TikTok content script loaded");const x=new ee;x.init();let _=location.href;new MutationObserver(()=>{location.href!==_&&(_=location.href,(location.pathname.includes("/foryou")||location.pathname.includes("/following")||location.pathname==="/")&&(x.destroy(),x.init()))}).observe(document.body,{subtree:!0,childList:!0})})();
